...
function parseStandardFeed(xmlString, sourceMeta) {
    ...
    try {
        ...
        if (channelLogoUrl) {
            // Some feeds parse <url> as an object with attributes — extract the string
            if (typeof channelLogoUrl === 'object') {
                if (channelLogoUrl['#text']) channelLogoUrl = channelLogoUrl['#text'];
                else if (channelLogoUrl.__cdata) channelLogoUrl = channelLogoUrl.__cdata;
                else channelLogoUrl = null;
            }
            if (channelLogoUrl && typeof channelLogoUrl === 'string') {
        ...
        for (const item of items) {
            ...
            if (typeof pubDate === 'object' && pubDate.__cdata) pubDate = pubDate.__cdata;
            let pDateTs = pubDate ? new Date(pubDate).getTime() : Date.now();
            if (isNaN(pDateTs)) pDateTs = Date.now();
            ...
            const acceptImage = (url) => {
                ...
                // Compare against channel-level logo
                if (channelLogoUrl) {
                    let normalizedUrl = url.trim().toLowerCase();
                    if (normalizedUrl.startsWith('//')) normalizedUrl = 'https:' + normalizedUrl;
                    // Exact URL match
                    if (normalizedUrl === channelLogoUrl) return null;
                    // Filename-based match (compare just the last path segment)
                    // This avoids false positives when article images share a path prefix
                    // e.g., channel logo: /wp-content/uploads/2025/10/logo.png
                    //        article img:  /wp-content/uploads/2026/04/article-image.png
                    try {
                        const urlFile = normalizedUrl.split('/').pop().split('?')[0];
                        const logoFile = channelLogoUrl.split('/').pop().split('?')[0];
                        if (urlFile && logoFile && urlFile === logoFile) return null;
                    } catch (_) {}
                }
                return url;
            };
            ...
            if (!imgUrl && item['media:content']) {
                ...
                if (mc['@_url']) {
                    imgUrl = acceptImage(mc['@_url']);
                } else if (mc['media:image'] && mc['media:image']['@_url']) {
                    imgUrl = acceptImage(mc['media:image']['@_url']);
                }
            }
            ...
            // 5. Extract from HTML content (try content:encoded FIRST, then description separately)
            // Some feeds put images only in content:encoded while description is plain text
            if (!imgUrl && item['content:encoded']) {
                const ceRaw = item['content:encoded'];
                const ceStr = typeof ceRaw === 'object' && ceRaw.__cdata ? ceRaw.__cdata : String(ceRaw);
                const ceImg = extractImageFromHtml(ceStr);
                imgUrl = acceptImage(ceImg);
            }
            if (!imgUrl && item.description) {
                const descRaw = item.description;
                const descStr = typeof descRaw === 'object' && descRaw.__cdata ? descRaw.__cdata : String(descRaw);
                const descImg = extractImageFromHtml(descStr);
                imgUrl = acceptImage(descImg);
            }
            ...
            // 9. item.image (fallback - used by OneIndia)
            if (!imgUrl && item.image) {
                const itemImg = typeof item.image === 'string' ? item.image : (item.image['#text'] || item.image['@_url'] || null);
                if (itemImg) imgUrl = acceptImage(itemImg);
            }
 ...
 * Google News feeds have redirect URLs, no images, and titles with " - Source" suffix
 */
function parseGoogleNewsFeed(xmlString, sourceMeta) {
    const articles = [];
    try {
        const result = parser.parse(xmlString);
        ...
        for (const item of items) {
            ...
            // Clean Google News title suffix like " - NDTV" or " - Times of India"
            title = title.replace(/\s*[-–—]\s*[^-–—]{2,30}$/, '').substring(0, 150);
            ...
            // Google News links are redirect URLs — they work fine on mobile click
            // No need to resolve them server-side

            let pubDate = item.pubDate || item.published;
            let pDateTs = pubDate ? new Date(pubDate).getTime() : Date.now();
            if (isNaN(pDateTs)) pDateTs = Date.now();

            // Extract real source name from <source> tag if available
            let realSource = sourceMeta.name;
            if (item.source) {
                const srcName = typeof item.source === 'string' ? item.source : (item.source['#text'] || item.source);
                if (srcName && typeof srcName === 'string' && srcName.length > 1) {
                    realSource = srcName.trim();
                }
            }
            ...
            if (title && link) {
                articles.push({
                    ...
                    d: description,
                    u: link,
                    i: null,  // Google News RSS has no images
                    s: realSource,
                    c: sourceMeta.category,
                    p: pDateTs,
                    l: sourceMeta.language,
        ...
        console.error("Google News parse error for " + sourceMeta.name, err.message);
    }
    return articles;
}

/**
 * Main entry — auto-detects YouTube vs Google News vs standard feed
 */
function parseFeed(xmlString, sourceMeta) {
    ...
    if (sourceMeta.url && sourceMeta.url.includes('news.google.com/rss')) {
        return parseGoogleNewsFeed(xmlString, sourceMeta);
    }
